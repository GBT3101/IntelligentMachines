...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        if len(legalActions)!=0:
          if util.flipCoin(self.epsilon): # p = epsilon
            action = random.choice(legalActions) # randomly choose an action
          else:
    ...
    def update(self, state, action, nextState, reward):
        ...
        computedValue = self.computeValueFromQValues(nextState)
        self.qvals[(state,action)] = ((1-a) * qValue) + a * (reward + d * computedValue)
        return
        util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in features.keys():
          self.weights[feature] = self.getWeights()[feature]+(self.alpha*Delta*features[feature])
        return
